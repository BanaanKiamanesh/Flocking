...
%% Simulate
SimTime = 120;
Motion  = F.Simulate(SimTime, @Traj2D);
...
[X, Y] = GetNeighbors(MHist(:, :, 1), Pos);

figure('Name', 'Agents', 'Units', 'normalized', 'OuterPosition', [0, 0, 1, 1]);
P1 = plot(X, Y, 'DisplayName', 'Link'); hold on
P2 = scatter(Pos(:, 1), Pos(:, 2), "filled", 'DisplayName', 'Agent');

Traj = Traj2D(Motion.t(1));
TrajLineX = xline(Traj(1), 'HandleVisibility', 'off');
TrajLineY = yline(Traj(2), 'HandleVisibility', 'off');

TrackPoint = scatter(Traj(1), Traj(2), 150, 'filled', 'ok', "DisplayName", 'Tracking Point');
...
for i = 2:5:numel(Motion.t)
    ...
    Traj = Traj2D(Motion.t(i));
    TrajLineX.Value = Traj(1);
    TrajLineY.Value = Traj(2);
